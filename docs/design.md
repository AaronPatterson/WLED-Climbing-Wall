...
| 14 | Clear the wall | Turn every hold off in one action, to start a fresh route |
...
- **Phase 14** is really a gap in Phase 3 rather than a new feature: starting a fresh route currently means tapping every lit hold one at a time. The plumbing already exists — `setHoldColors` with nothing lit clears the wall, and is already covered by a test — so this is a button and a ViewModel method. Worth pulling forward.
